...
+------------------------------------+
	...
	for j in range(2, len(numbers)-3):
		...
		if len(allCombinationsFirstGroup) == 0:
			...
			for currCombination in allCombinationsFirstGroup:
				...
				for i in range(len(residualNumbers)-2, 1, -1):
					...
					for currSecondCombination in allCombinationsSecondGroup:
						...
						residualNumbersThird = [item for item in residualNumbers if item not in list(currSecondCombination)]
						# Iterate over all possible amounts of packages in the third group
						for k in range(1, len(residualNumbers)-1):
							if validCombinationsFound:
								break

							# Get the valid combinations for the third group with amount of packages k
							allCombinationsThirdGroup = getCombinationsWithSum(residualNumbersThird, k, targetSum)
							if len(allCombinationsThirdGroup) == 0:
								continue
							# Iterate over all valid combinations for the third group
							for currThirdCombination in allCombinationsThirdGroup:
								...
								residualNumbersLast = [item for item in residualNumbersThird if item not in list(currThirdCombination)]
