...
#          Functions           #
#------------------------------#
def playGame(playerStats, opponentStats, castOrder, spellbook, verbose):
...
+------------------------------------+
	...
	while True:
		...
		currTotalManaSpentMinimum = float('inf')
		currLowestWinningCastOrder = []
		...
		for currCastOrder in allCastCombinations:
			...
			gameOutComeState, totalManaSpent, castSpells = playGame(playerStats, opponentStats, castOrder, spellbook, False)
			if gameOutComeState==0:
				...
				if totalManaSpent <= currTotalManaSpentMinimum:
					currTotalManaSpentMinimum = totalManaSpent
					if totalManaSpent == currTotalManaSpentMinimum:
						currLowestWinningCastOrder.append(castOrder)
						orderString = 's'
					else:
						currLowestWinningCastOrder = [castOrder]
						orderString = ''
		...
		if numberOfGamesPlayerWon > 0:
			print('Lowest amount of Mana spent for a victory: {}'.format(currTotalManaSpentMinimum))
			print('Lowest-Mana winning cast order{}: {}'.format(orderString, currLowestWinningCastOrder))
		print()
		print()
	...
	print('--------------------')
	print('Overall lowest amount of Mana spent for a victory: {}'.format(totalManaSpentMinimum))
