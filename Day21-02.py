	...
	for i in range(len(shop['Weapons'])):
		...
		for j in range(-1, len(shop['Armor'])):
			...
			for k in range(-1, len(allRingCombinations)):
				...
				# Calculate the winner and update the cost of the losing configuration if it is higher than the current highest value
				if not calcWinner(currPlayerStats, opponentStats):
					if currCost > losingCost:
						losingCost = currCost
						winningConfigName = currConfigName
						winningConfig = currPlayerStats.copy()

	print('The most amount of gold required for still losing is given by {}.'.format(losingCost))
